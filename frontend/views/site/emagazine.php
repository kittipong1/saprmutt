<?php 
Yii::setAlias('@kmpath', '@web');

?><!doctype html>
<!--[if IE 8 ]><html class="ie ie8" lang="en"> <![endif]-->
<!--[if (gte IE 9)|!(IE)]><html lang="en" class="no-js"> <![endif]-->
<html lang="en">



<body>

  <!-- Container -->
  <div id="container">

    <!-- Start Header Section -->

    <!-- End Header Section -->


    <!-- Start Page Banner -->
    <div class="page-banner no-subtitle">
      <div class="container">
        <div class="row">
          <div class="col-md-6">
            <h2>วารสาร (E-magazine)</h2>
          </div>
          <div class="col-md-6">
            <ul class="breadcrumbs">
              <li><a href="index.php">หน้าหลัก</a></li>
              <li>วารสาร (E-magazine)</li>
            </ul>
          </div>
        </div>
      </div>
    </div>
    <!-- End Page Banner -->


    <!-- Start Content -->
    <div id="content" class="bg-white">
      <div class="container">
        <div class="row sidebar-page">


          <!-- Page Content -->
          <div class="col-md-9 page-content">

                  <!-- Tab panels -->
                  <!-- <div class="tab-content"> -->

                    <!-- <div class="row"> -->

                      <!-- Start Recent Projects Carousel -->
                      <ul id="portfolio-list" data-animated="fadeIn">
                        <li class="gallery_product filter eb1">
                         <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb2">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb3">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb4">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb4">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div> 
                        </li>
                        <li class="gallery_product filter eb3">
                         <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb2">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb1">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div> 
                        </li>
                        <li class="gallery_product filter eb1">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div>  
                        </li>
                        <li class="gallery_product filter eb2">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div> 
                        </li>
                        <li class="gallery_product filter eb3">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div> 
                        </li>
                        <li class="gallery_product filter eb4">
                          <img src="images/img-book.png" alt="" />
                          <!--<div class="portfolio-item-content">
                            <span class="header">ชื่อวารสาร...</span>
                            <p class="body animated8">รายละเอียดวารสาร.................................</p>
                            <p class="body animated8">ชื่อหน่วยงาน...</p>
                          </div> -->
                          <p> </p>
                            <h5>ชื่อเอกสาร...</h5>
                            <h5>ชื่อหน่วยงาน...</h5>
                            <p><small><i class="fa fa-calendar"></i> 24/08/2560</small></p>
                          <div class="portfolio-item-footer">
                            <p><i class="fa fa-download"></i> จำนวนการดาวน์โหลด <span class="pull-right">100 ครั้ง</span></p>
                          </div>
                          <div class="icon-list">
                            <a class="link" href="single-project.html"><i class="fa fa-link"></i></a>
                            <a class="zoom lightbox" href="images/img-book.png"><i class="fa fa-search-plus"></i></a>
                          </div> 
                        </li>
                      </ul>
                      <!-- End Recent Projects Carousel -->
                      <!-- </div> -->

                  <!-- </div> -->
                  <!-- End Tab Panels -->

                  <div class="hr1" style="margin-bottom:30px;"></div>

                  <!-- Start Pagination -->
                  <div id="pagination">
                    <span class="all-pages">หน้าที่ 1 จาก 3</span>
                    <span class="current page-num">1</span>
                    <a class="page-num" href="#">2</a>
                    <a class="page-num" href="#">3</a>
                    <a class="next-page" href="#">ถัดไป</a>
                  </div>
                  <!-- End Pagination -->

                </div>
          <!-- End Page Content-->


          <!--Sidebar-->
          <div class="col-md-3 sidebar right-sidebar">

            <!-- Categories Widget -->
            <div class="tabs-section widget">

              <!-- Nav Tabs -->
              <ul class="nav nav-tabs">
                <li class="active"><a href="#" data-toggle="tab"><i class="fa fa-share-alt"></i>ประเภทวารสาร</a></li>
                <!-- <span class="pull-right" style="padding: 8px 0px;"><a href="#" class="btn btn-warning btn-sm" title="ดูทั้งหมด"><i class="fa fa-plus"></i></a></span> -->
              </ul>

              <!-- Tab panels -->
              <div class="tab-content widget-categories">
                <!-- Tab Content 1 -->
                <div class="tab-pane fade in active">
                  <ul>
                    <li>
                      <a href="#" class="filter-button" data-filter="all">ทั้งหมด</a>
                    </li>
                    <li>
                      <a href="#" class="filter-button" data-filter="eb1">ประเภทที่ 1</a>
                    </li>
                    <li>
                      <a href="#" class="filter-button" data-filter="eb2">ประเภทที่ 2</a>
                    </li>
                    <li>
                      <a href="#" class="filter-button" data-filter="eb3">ประเภทที่ 3</a>
                    </li>
                    <li>
                      <a href="#" class="filter-button" data-filter="eb4">ประเภทที่ 3</a>
                    </li>
                  </ul>
                </div>
              </div>

            <!-- Popular Posts widget -->
            <div class="tabs-section widget">

              <!-- Nav Tabs -->
              <ul class="nav nav-tabs">
                <li class="active"><a href="#" data-toggle="tab"><i class="fa fa-star"></i>วารสารยอดนิยม</a></li>
                <!-- <span class="pull-right" style="padding: 8px 0px;"><a href="#" class="btn btn-warning btn-sm" title="ดูทั้งหมด"><i class="fa fa-plus"></i></a></span> -->
              </ul>

              <!-- Tab panels -->
              <div class="tab-content widget-popular-posts">
                <!-- Tab Content 1 -->
                <div class="tab-pane fade in active">
                  <ul>
                    <?php  
                      for ($x = 0; $x <= 4; $x++) {
                        echo '<li>
                      <div class="widget-thumb">
                        <a href="#"><img src="images/img-book.png" alt="" /></a>
                      </div>
                      <div class="widget-content">
                        <h5><a href="#">ชื่อวารสาร...</a></h5>
                        <span><i class="fa fa-calendar"></i> : 29/08/2560</span> <br>
                        <span><i class="fa fa-download"></i> : 123 ครั้ง</span>
                      </div>
                      <div class="clearfix"></div>
                    </li>';
                      }
                    ?> 
                  </ul>
                </div>
              </div>
              <!-- End Tab Panels -->

            </div>

            <!-- Tags Widget -->
            <div class="tabs-section widget widget-tags">

              <!-- Nav Tabs -->
              <ul class="nav nav-tabs">
                <li class="active"><a href="#" data-toggle="tab"><i class="fa fa-tag"></i>แท็ก</a></li>
              </ul>

              <!-- Tab panels -->
              <div class="tab-content">
                <!-- Tab Content 1 -->
                <div class="tab-pane fade in active">
                  <div class="tagcloud">
                    <a href="#">แท็ก1</a>
                    <a href="#">แท็ก2</a>
                    <a href="#">แท็ก3</a>
                    <a href="#">แท็ก4</a>
                    <a href="#">แท็ก5</a>
                    <a href="#">แท็ก6</a>
                    <a href="#">แท็ก7</a>
                    <a href="#">แท็ก8</a>
                    <a href="#">แท็ก9</a>
                    <a href="#">แท็ก10</a>
                    <a href="#">แท็ก11</a>
                  </div>
                </div>
              </div>
              <!-- End Tab Panels -->

            </div>

          </div>
          <!--End sidebar-->


        </div>
      </div>
    </div>
    </div>
    <!-- End Content -->


    <!-- Start Footer Section -->

    <!-- End Footer Section -->

  </div>
  <!-- End Container -->


</body>

</html>