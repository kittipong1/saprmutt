...
/**
 * CheckstudentbymajorSearch represents the model behind the search form about `app\models\studen`.
 */
class CheckstudentbymajorSearch extends studen
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        $major_id = Information::find()->where(['user_id'=>Yii::$app->user->identity->id])->one();
       
        if(!empty($major_id)){
             $query = studen::find()->where(['major_id'=>$major_id->major_id]);
         }else {
            echo '<script>
                alert("ไม่มีนักศึกษาในสาขา");
